  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl p-6">
        <h2 className="text-base font-semibold mb-1" style={{ color: "#F8FAFC" }}>Program Type</h2>
        <p className="text-xs mb-4" style={{ color: "rgba(248,250,252,0.5)" }}>Tell us which post-bacc path you&apos;re on.</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {PROGRAM_TYPES.map((pt) => {
            ...
            return (
              ...
              >
                <p className="text-sm font-bold mb-0.5" style={{ color: active ? "#00D4FF" : "#F8FAFC" }}>{pt.label}</p>
                <p className="text-xs" style={{ color: "rgba(248,250,252,0.5)" }}>{pt.desc}</p>
              </button>
            );
          })}
        </div>
      ...
      {/* Add course form */}
      <div className="glass-card rounded-2xl p-6">
        <h2 className="text-base font-semibold mb-4" style={{ color: "#F8FAFC" }}>Add a Course</h2>
        <form onSubmit={handleAddCourse} className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-[120px,140px,1fr,auto] gap-3 items-center">
            ...
            <label className="inline-flex items-center gap-2 text-sm cursor-pointer" style={{ color: "rgba(248,250,252,0.85)" }}>
              <input
                type="checkbox"
                checked={cBcpm}
      ...
      {/* Course list */}
      <div className="glass-card rounded-2xl p-6">
        <h2 className="text-base font-semibold mb-4" style={{ color: "#F8FAFC" }}>Your Courses</h2>
        {initialCourses.length === 0 ? (
          <p className="text-sm text-center py-8" style={{ color: "rgba(248,250,252,0.5)" }}>
            ...
            <table className="w-full text-sm" style={{ minWidth: 600 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(0,212,255,0.15)" }}>
                  <th className="text-left px-2 py-2 text-xs uppercase tracking-wide" style={{ color: "rgba(248,250,252,0.5)" }}>Course</th>
                  <th className="text-left px-2 py-2 text-xs uppercase tracking-wide" style={{ color: "rgba(248,250,252,0.5)" }}>Semester</th>
                  <th className="text-right px-2 py-2 text-xs uppercase tracking-wide" style={{ color: "rgba(248,250,252,0.5)" }}>Credits</th>
                  <th className="text-center px-2 py-2 text-xs uppercase tracking-wide" style={{ color: "rgba(248,250,252,0.5)" }}>Grade</th>
                  <th className="text-center px-2 py-2 text-xs uppercase tracking-wide" style={{ color: "rgba(248,250,252,0.5)" }}>BCPM</th>
                  <th className="px-2 py-2"></th>
                </tr>
              </thead>
              <tbody>
                {initialCourses.map((c) => (
                  <tr key={c.id} style={{ borderBottom: "1px solid rgba(0,212,255,0.06)" }}>
                    <td className="px-2 py-3" style={{ color: "#F8FAFC" }}>{c.name}</td>
                    <td className="px-2 py-3" style={{ color: "rgba(248,250,252,0.65)" }}>{c.semester || "—"}</td>
                    <td className="px-2 py-3 text-right" style={{ color: "rgba(248,250,252,0.85)" }}>{c.credits}</td>
                    <td className="px-2 py-3 text-center">
                      <span className="inline-flex items-center justify-center w-9 px-2 py-0.5 rounded-md text-xs font-bold" style={{ background: "rgba(0,212,255,0.12)", color: "#00D4FF", border: "1px solid rgba(0,212,255,0.3)" }}>
                        {c.grade}
                      </span>
                    </td>
                    <td className="px-2 py-3 text-center">
                      {c.is_bcpm ? (
                        <svg className="w-4 h-4 mx-auto" fill="none" stroke="#00D4FF" strokeWidth="2.5" viewBox="0 0 24 24">
      ...
      {/* Notes */}
      <div className="glass-card rounded-2xl p-6">
        <h2 className="text-base font-semibold mb-1" style={{ color: "#F8FAFC" }}>Notes</h2>
        <p className="text-xs mb-3" style={{ color: "rgba(248,250,252,0.5)" }}>Track strategy, advisor meetings, anything else.</p>
        <textarea
          value={profile.notes ?? ""}
  ...
  return (
    <div className="glass-card rounded-2xl p-5">
      <div className="flex items-center justify-between mb-1">
        <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "rgba(0,212,255,0.7)" }}>{label}</p>
        {value !== null && (
          <span className="text-[10px] font-bold uppercase tracking-wide" style={{ color: metGoal ? "#10B981" : "rgba(248,250,252,0.5)" }}>
            {metGoal ? "Goal Met ✓" : `${pct.toFixed(0)}% to goal`}
          </span>
        )}
      </div>
      <div className="flex items-baseline gap-2 mb-3">
        <span className="text-4xl font-bold" style={{ color: "#00D4FF" }}>{formatGpa(value)}</span>
        <span className="text-sm" style={{ color: "rgba(248,250,252,0.5)" }}>/ 4.00</span>
      </div>

      <div className="w-full h-2 rounded-full overflow-hidden mb-3" style={{ background: "rgba(0,212,255,0.08)" }}>
        <div
          className="h-full rounded-full"
          style={{
            width: `${pct}%`,
            background: metGoal ? "#10B981" : "#00D4FF",
            boxShadow: `0 0 8px ${metGoal ? "rgba(16,185,129,0.5)" : "rgba(0,212,255,0.5)"}`,
            transition: "width 0.4s",
          }}
        />
      </div>

      <p className="text-xs mb-2" style={{ color: "rgba(248,250,252,0.5)" }}>{sub}</p>

      <div className="flex items-center gap-2 pt-2" style={{ borderTop: "1px solid rgba(0,212,255,0.08)" }}>
        <label className="text-xs" style={{ color: "rgba(248,250,252,0.6)" }}>Goal:</label>
        <input

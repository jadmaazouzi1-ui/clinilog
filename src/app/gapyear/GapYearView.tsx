...
const CATEGORIES: { value: GapGoal["category"]; label: string; color: string }[] = [
  { value: "clinical",     label: "Clinical Hours",   color: "#000000" },
  { value: "research",     label: "Research",         color: "#000000" },
  { value: "volunteering", label: "Volunteering",     color: "#000000" },
  { value: "personal",     label: "Personal Growth",  color: "#000000" },
  { value: "financial",    label: "Financial",        color: "#000000" },
];
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex-1 min-w-[200px]">
            <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: "rgba(0,0,0,0.55)" }}>Target Application Cycle</p>
            <select
          ...
          {countdown && (
            <div
              ...
              style={{ background: "#FFFFFF", border: "2px solid #000000", minWidth: 180 }}
            >
              <p className="text-3xl font-bold" style={{ color: "#000000" }}>{countdown.days}</p>
              <p className="text-xs mt-1" style={{ color: "rgba(0,0,0,0.6)" }}>days until {countdown.label}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            <h2 className="text-base font-semibold" style={{ color: "#000000" }}>Gap Year Goals</h2>
            <p className="text-xs" style={{ color: "rgba(0,0,0,0.5)" }}>{data.goals.length}/5 set</p>
          </div>
        </div>
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{ background: "#FFFFFF", color: "rgba(0,0,0,0.6)", border: "2px solid #000000" }}
                  >
                    {meta.label}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium" style={{ color: "#000000" }}>{g.title}</p>
                    {g.target && <p className="text-xs mt-0.5" style={{ color: "rgba(0,0,0,0.6)" }}>Target: {g.target}</p>}
                  </div>
                  <button
      ...
      {/* Monthly log */}
      <div className="glass-card rounded-2xl p-6">
        <h2 className="text-base font-semibold mb-1" style={{ color: "#000000" }}>Monthly Activity Log</h2>
        <p className="text-xs mb-4" style={{ color: "rgba(0,0,0,0.5)" }}>What did you do this month? Track progress on your goals.</p>
        ...
        {sortedLogEntries.length === 0 ? (
          <p className="text-xs text-center py-4" style={{ color: "rgba(0,0,0,0.4)" }}>No entries logged yet.</p>
        ) : (
          <ul className="space-y-2">
            {sortedLogEntries.map(([month, text]) => (
              <li
                ...
                style={{ background: "#FFFFFF", border: "2px solid #000000" }}
              >
                <span className="text-xs font-bold uppercase tracking-wide flex-shrink-0" style={{ color: "#000000", minWidth: 76 }}>
                  {formatMonth(month)}
                </span>
                <p className="text-sm flex-1" style={{ color: "rgba(0,0,0,0.85)" }}>{text}</p>
                <button onClick={() => deleteLog(month)} className="p-1 opacity-50 hover:opacity-100" style={{ color: "#000000" }} aria-label="Delete log">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            <h2 className="text-base font-semibold" style={{ color: "#000000" }}>Application Milestones</h2>
            <p className="text-xs" style={{ color: "rgba(0,0,0,0.5)" }}>{milestonesDone}/{MILESTONES.length} complete</p>
          </div>
            ...
            return (
              <li key={m.id}>
                <button
                  ...
                  style={{
                    background: done ? "rgba(0,0,0,0.08)" : "rgba(0,0,0,0.02)",
                    border: `1px solid ${done ? "rgba(0,0,0,0.12)" : "rgba(0,0,0,0.08)"}`,
                  }}
                >
                  <span
                    className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5"
                    style={{
                      background: done ? "#000000" : "transparent",
                      border: `1.5px solid ${done ? "#000000" : "rgba(0,0,0,0.3)"}`,
                    }}
                  >
                    {done && (
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      ...
                      style={{ color: done ? "rgba(0,0,0,0.55)" : "#000000", textDecoration: done ? "line-through" : "none" }}
                    >
                      {m.label}
                    </p>
                    <p className="text-xs mt-0.5" style={{ color: "rgba(0,0,0,0.4)" }}>{m.due}</p>
                  </div>
                </button>
              </li>
            );

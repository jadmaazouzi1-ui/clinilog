...
function RatingDots({ value, color, label }: { value: Rating; color: string; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: "rgba(248,250,252,0.5)", minWidth: 96 }}>{label}</span>
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <span
            ...
            style={{
              backgroundColor: i <= value ? color : "rgba(0,212,255,0.12)",
              boxShadow: i <= value ? `0 0 6px ${color}55` : "none",
            }}
          />
        ))}
      </div>
      <span className="text-[10px]" style={{ color: "rgba(248,250,252,0.45)" }}>{value}/5</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <p className="text-sm mb-4 font-medium" style={{ color: "rgba(248,250,252,0.5)" }}>
        Showing <span className="font-semibold" style={{ color: "#F8FAFC" }}>{filtered.length}</span> of {SPECIALTIES.length} specialties
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {filtered.length === 0 && (
          <div className="col-span-full glass-card rounded-2xl p-8 text-center text-sm" style={{ color: "rgba(248,250,252,0.5)" }}>
...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
  return (
    <div>
      <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "rgba(248,250,252,0.5)" }}>{label}</label>
      <select
        value={value}
...
function Card({ specialty: s, expanded, onToggle }: { specialty: Specialty; expanded: boolean; onToggle: () => void }) {
  return (
    <div
      ...
      style={{ borderColor: expanded ? "rgba(0,212,255,0.5)" : undefined }}
      onClick={onToggle}
    >
      <div className="flex items-start gap-4">
        ...
        >
          <svg className="w-5 h-5" fill="none" stroke="#00D4FF" strokeWidth="1.75" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-bold leading-tight" style={{ color: "#F8FAFC" }}>{s.name}</h3>
            <svg
              className="w-4 h-4 flex-shrink-0 transition-transform"
              style={{ transform: expanded ? "rotate(180deg)" : "none", color: "rgba(248,250,252,0.4)" }}
              fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"
            >
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
          </div>
          <div className="flex items-center gap-2 flex-wrap mt-1.5 text-xs" style={{ color: "rgba(248,250,252,0.55)" }}>
      ...
      {expanded && (
        <div className="mt-5 pt-5 space-y-4" style={{ borderTop: "1px solid rgba(0,212,255,0.1)" }}>
          <div className="space-y-2">
            <RatingDots value={s.competitiveness} color="#FF8A8A" label="Competitive" />
            <RatingDots value={s.lifestyle} color="#00D4FF" label="Lifestyle" />
          </div>

          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wide mb-1.5" style={{ color: "rgba(0,212,255,0.7)" }}>Day in the Life</p>
            <p className="text-xs leading-relaxed" style={{ color: "rgba(248,250,252,0.8)" }}>{s.summary}</p>
          </div>

          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wide mb-1.5" style={{ color: "rgba(0,212,255,0.7)" }}>Top Schools for This Path</p>
            <ul className="space-y-1.5">
              {s.topSchools.map((school) => (
                <li key={school} className="text-xs flex items-start gap-2" style={{ color: "rgba(248,250,252,0.75)" }}>
